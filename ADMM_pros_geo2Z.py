...
#            m.addConstr(sum(self.goal[range(t+j,t+j+self.window)] - self.variables.load[range(t+j,t+j+self.window)]) == 0)
    ...
    def _build_objective_(self):
        ...
        if self.params.idx in range(0,10):
            res0 = self.MP.variables.r_k[range(0,24)] + self.MP.weight[0]*(self.variables.q - self.q_old)
            price0 = self.MP.variables.price_comm[temp,0]
        if self.params.idx in range(10,15):
            res0 = self.MP.variables.r_k[range(24,48)] + self.MP.weight[1]*(self.variables.q - self.q_old)
            price0 = self.MP.variables.price_comm[temp,1]
        res1 = self.MP.variables.r_k[range(48,72)] + (self.variables.alfa - self.alfa_old)
        res2 = self.MP.variables.r_k[range(72,96)] + self.variables.beta - self.beta_old
