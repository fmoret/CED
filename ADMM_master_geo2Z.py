...
class ADMM_Master:
    def __init__(self, b1, c1, Pmin, Pmax, PV, b2, c2, Load, Flex_load, tau, el_price_e, o, e, time, window, case, threshold, gamma_geo):
        ...
        self.variables.pow_comm = np.zeros(time)
        self.variables.price_comm = np.zeros((time,2))
        self.variables.price_IE = np.zeros((time,2))
        
        self.imbalance = np.zeros((time+1, self.params.num_pros))
        self.iter_max = 10000
        self.weight = np.array([2/self.params.num_pros,3/self.params.num_pros])
...
#        sys.stdout = open('pros_out.txt','w')
    ...
    def _build_variables(self):
        ...
        self.variables.q_comm = np.array([m.addVar() for k in range(24)])
        self.variables.q_cpp = np.array([m.addVar() for k in range(24)])
        for k in range(24):
            m.addConstr(self.variables.q_comm[k] <= self.variables.q_cpp[k])
            m.addConstr(self.variables.q_comm[k] >= -self.variables.q_cpp[k])
        m.update()
    ...
    def _build_objective(self, t):
        ...
        o = self.data.sigma
        res01 = self.weight[0]*(q[:,range(0,10)].sum(axis=1)-self.variables.q_comm)
        res02 = self.weight[1]*(q[:,range(10,15)].sum(axis=1)+self.variables.q_comm)
        res1 = alfa.sum(axis=1) - self.variables.q_imp
        res2 = beta.sum(axis=1) - self.variables.q_exp
        
        self.objective = (sum(pr_imp*self.variables.q_imp + pr_exp*self.variables.q_exp) + sum(self.variables.q_cpp*gamma_geo)
                          + sum(price_0[:,0]*res01+price_0[:,1]*res02) + sum(price_1[:,0]*res1+price_1[:,1]*res2) + o/2*(sum(res01*res01+res02*res02)+sum(res1*res1+res2*res2)))
    ...
    def optimize(self):
        ...
        for t in np.arange(0,self.params.time,24): #8,self.params.time
            ...
            self.variables.r_k = np.ones(4*24)#+self.params.num_pros)
            real_rk = np.ones(4*24)
            ...
            while (np.linalg.norm(real_rk)>self.data.tol or self.variables.s_k>self.data.tol) and self.variables.iter<self.iter_max and np.linalg.norm(real_rk)<10**10: #np.linalg.norm
                ...
                for k in range(24):
                    ...
                    self.variables.pow_comm[t+k] = self.variables.q_comm[k].x

                #Calculate residulas
#                print(self.variables.q_k[self.temp,:][:,range(0,5)])
#                print(self.variables.q_k[self.temp,range(0,5)].sum(axis=1))
                self.variables.r_k[range(0,24)] = self.weight[0]*(self.variables.q_k[self.temp,:][:,range(0,10)].sum(axis=1)-self.variables.pow_comm[self.temp])
                self.variables.r_k[range(24,48)] = self.weight[1]*(self.variables.q_k[self.temp,:][:,range(10,15)].sum(axis=1)+self.variables.pow_comm[self.temp])
                self.variables.r_k[range(48,72)] = (self.variables.alfa_k[self.temp,:].sum(axis=1) - self.variables.pow_imp[self.temp])
                self.variables.r_k[range(72,96)] = self.variables.beta_k[self.temp,:].sum(axis=1) - self.variables.pow_exp[self.temp]
                ...
                Dcomm = (self.variables.pow_comm[self.temp] - q_comm_old)
                Dimp = self.variables.pow_imp[self.temp]-imp_old
                Dexp = self.variables.pow_exp[self.temp]-exp_old
                self.variables.s_k = sqrt(self.params.num_pros)*self.data.sigma*sqrt(sum(Dimp*Dimp + Dexp*Dexp + Dcomm*Dcomm))

                real_rk = np.copy(self.variables.r_k)
                real_rk[range(0,24)] = real_rk[range(0,24)]/self.weight[0]
                real_rk[range(24,48)] = real_rk[range(24,48)]/self.weight[1]
                ...
                self.variables.price_IE[self.temp,0] = self.variables.price_IE[self.temp,0] + self.data.sigma*self.variables.r_k[range(48,72)]
                self.variables.price_IE[self.temp,1] = self.variables.price_IE[self.temp,1] + self.data.sigma*self.variables.r_k[range(72,96)]
...
#                    bb = {'q_12' : self.variables.pow_comm[t+k]}
...
#                    res10.append(ii)
            ...
            print('Geo 2 Zones - Timestamp %g' % (t), '  Flag ', flag)
