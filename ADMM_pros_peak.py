...
#            m.addConstr(sum(self.goal[range(t+j,t+j+self.window)] - self.variables.load[range(t+j,t+j+self.window)]) == 0)
    ...
    def _build_objective_(self):
        ...
        price3 = self.MP.variables.price_max[temp]
        ...
        res3 = self.MP.variables.r_k[range(72,96)] + self.MP.weight*(self.variables.alfa - self.alfa_old)
        self.objective = (sum(y0_c*(self.variables.load) + mm_c/2*(self.variables.load*self.variables.load) + y0_g*(self.variables.power) + mm_g/2*(self.variables.power*self.variables.power))
                          + sum(0.001*self.variables.q_pos) + sum(price0*res0) + sum(price1[:,0]*res1+price1[:,1]*res2+price3*res3) + o/2*(sum(res0*res0)+sum(res1*res1+res2*res2+res3*res3))) #+ sum(0.1*self.variables.alfa + 0.05*self.variables.beta 
