...
from ADMM_pros_peak2 import ADMM_Pros
...
#        sys.stdout = open('pros_out.txt','w')
    ...
    def _build_variables(self):
        ...
        self.variables.delta = np.array([m.addVar() for k in range(24)])
        self.variables.theta = m.addVar()
        for k in range(24):
            m.addConstr(self.variables.delta[k] - self.variables.theta <= 0)
        m.update()
    ...
    def _build_objective(self, t):
        ...
        price_3 = self.variables.price_max[self.temp]
        o = self.data.sigma
        gamma = 1.0
        res0 = self.weight*q.sum(axis=1)
        res1 = alfa.sum(axis=1) - self.variables.q_imp
        res2 = beta.sum(axis=1) - self.variables.q_exp
        res3 = self.weight*(alfa.sum(axis=1) - self.variables.delta)

        self.objective = (gamma*self.variables.theta + sum(pr_imp*self.variables.q_imp + pr_exp*self.variables.q_exp + price_0*res0) 
                          + sum(price_1[:,0]*res1+price_1[:,1]*res2+price_3*res3) + o/2*(sum(res0*res0)+sum(res1*res1+res2*res2+res3*res3)))
    ...
    def optimize(self):
        ...
        for t in np.arange(0,self.params.time,24): #8,self.params.time
            ...
            while (np.linalg.norm(real_rk)>self.data.tol or self.variables.s_k>self.data.tol) and self.variables.iter<10000 and np.linalg.norm(real_rk)<10**10: #np.linalg.norm
                ...
                delta_old = self.variables.pow_delta[self.temp]
                for k in range(24):
                    self.variables.pow_imp[t+k] = self.variables.q_imp[k].x
                    self.variables.pow_exp[t+k] = self.variables.q_exp[k].x
                    self.variables.pow_delta[t+k] = self.variables.delta[k].x
                    self.variables.pow_theta[t+k] = self.variables.theta.x
                ...
                self.variables.r_k[range(72,96)] = self.weight*(self.variables.alfa_k[self.temp,:].sum(axis=1) - self.variables.pow_delta[self.temp])
                ...
                Dimp = self.variables.pow_imp[self.temp]-imp_old + self.weight*(self.variables.pow_delta[self.temp]-delta_old)
                ...
                real_rk[range(72,96)] = real_rk[range(72,96)]/self.weight
                ...
                self.variables.price_max[self.temp] = self.variables.price_max[self.temp] + self.data.sigma*self.variables.r_k[range(72,96)]
